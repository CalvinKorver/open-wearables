...
- Inbound DMs: `/brief`, `/goal`, `/fact`, `/memory`, `/forget`, `/clear`, `/help`, or a natural-language question
- Durable goals and facts (survive `/clear` and restarts); recent multi-turn chat memory (last 20 messages)
...
| `/goal <text>` | Save a durable goal (used in chat + briefings) |
| `/fact <text>` | Save a durable fact |
| `/goals` / `/facts` / `/memory` | List durable memory |
| `/forget <id>` / `/forget goals\|facts\|all` | Remove durable memory |
| `/clear` | Drop recent conversation turns (not goals/facts) |

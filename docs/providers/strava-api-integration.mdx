...
<Steps>
  ...
  <Step title="Configure credentials in Open Wearables">
    **Local development:** add the following to `backend/config/.env`:

    **Production (e.g. [Railway](/deployment/railway)):** set the same variables on the **Backend** service (not the Frontend), set `API_BASE_URL` to your public Backend URL, then redeploy.

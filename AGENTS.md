...
9. **Use valid PR titles** - PR titles must follow [Conventional Commits](https://www.conventionalcommits.org/) (`type(scope): description`). CI validates titles via `.github/workflows/pr-validation.yml`. Allowed types: `feat`, `fix`, `docs`, `style`, `refactor`, `perf`, `test`, `build`, `ci`, `chore`, `revert`. Optional scopes include `backend`, `frontend`, `docs`, `api`, `mcp`, `auth`, `integrations`, `dashboard`, `settings`, `users`. Examples: `fix(backend): reduce sleep finalize gap to 30 minutes`, `feat(api): add pagination to workouts endpoint`. See `contributing/pull-requests.md`.
